...
Структура каталога, фильтры, добавление и удаление карточек и выбор района адаптированы из предоставленного JavaScript. Первый экран оформлен по визуальному образцу: крупная типографика, локальная SVG-панорама Астаны и строка исходных показателей. Ниже расположены обзор дефицитов и рабочая область с планом справа. На узких экранах блоки перестраиваются в одну колонку. Расчеты и состояние перенесены в Python / Streamlit. Пять слайдеров и интеграция OpenAI сохранены.
...
Первый экран показывает исходный Score **52.56**, бюджет **100 ед.** и пять районов. Кнопка «Собрать сценарий» переводит к рабочей области, а «Сбросить сценарий» очищает оба плана и AI-отчеты.

Сразу под первым экраном расположен блок «Текущее состояние города и дефициты»: пять компактных карточек со слабейшими показателями транспорта, озеленения или соцсферы. Подробная таблица и начальные рекомендации по каталогу раскрываются по нажатию. Значения ниже 40 отмечены как критические, 40–49 — как дефицит для диагностики. Порог 50 используется только в этом обзоре и не меняет расчет Score. Блок всегда описывает исходный учебный датасет, а не прогноз выбранного сценария или реальные оперативные сведения о городе.
...
src/landing.py            первый экран, базовые метрики и карточки дефицитов
...
assets/landing.css        первый экран, рабочая область и адаптивная сетка
assets/city-hero.svg      локальная векторная панорама Астаны
...
Тесты не отправляют запросы внешним сервисам и не расходуют API-квоты. Для проверки реальных AI-ответов требуются рабочий ключ и доступ сервера к OpenAI. При обновлении интерфейса дополнительно проверены бюджет в 100 единиц и общий сброс обоих режимов.
