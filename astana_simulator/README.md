...
Веб-приложение на Python и Streamlit для кейса HackAlem AI: распределите **1 000 000 000 ₸**, оцените последствия для пяти условных районов и получите независимые объяснения от OpenAI и NVIDIA.
...
## Подключение настоящих AI-сервисов
...
Ключ OpenAI: <https://platform.openai.com/api-keys>. Ключ NVIDIA: <https://build.nvidia.com/>. Доступ к моделям, квоты и оплата определяются вашим аккаунтом. Не отправляйте ключи в чат и не добавляйте `.env` в Git — файл уже исключен через `.gitignore`.

OpenAI всегда вызывается с **`gpt-4o-mini`** по адресу `https://api.openai.com/v1`. NVIDIA вызывается через тот же пакет `openai` по **`https://integrate.api.nvidia.com/v1`**; модель можно заменить переменной `NVIDIA_MODEL`. По умолчанию выбран `nvidia/nemotron-3-super-120b-a12b`; для него отключено дополнительное рассуждение, чтобы быстрее получить экспресс-оценку. Для другой модели специфический параметр не отправляется.

Нажмите «Обновить подключение» в боковой панели после изменения `.env`, затем «Получить AI-разбор» под результатами. Переменные окружения имеют приоритет перед `.env`. Статус «ключ задан» означает наличие значения, а не проверку доступа.

- Два `AsyncOpenAI`-клиента запускаются параллельно через `asyncio.gather`.
- OpenAI пишет сильные стороны, скрытые риски, корректировки и вердикт.
- NVIDIA независимо анализирует инфраструктурные риски каждого района.
- Оба получают только синтетические исходные данные, бюджет, результаты, дельты и вклад мер. Ключи в содержимое запросов и экспорт не попадают.
...
src/ai.py                 настройки, два API-клиента, параллельные запросы
...
- [NVIDIA Nemotron API](https://docs.api.nvidia.com/nim/reference/nvidia-nemotron-3-super-120b-a12b-infer)
